...
void rtc_config()
{
    ...
    dynamic_attachInterrupt(RTC1_IRQn, RTC1_Interrupt);
...
void UART0_Start( int dwBaudRate, uint8_t rx_pin, uint8_t tx_pin, uint8_t cts_pin, uint8_t rts_pin )
{
  // must STOP before restarting
  if (UART0_State != UART0_State_NotStarted)
    return;

  // initialize tx output pin state high (uart is active low)
  ...
  dynamic_attachInterrupt(UART0_IRQn, UART0_Interrupt);
  ...
  {
    if (/* SimbleeBLE_enabled && */ dwBaudRate > 9600)
    {
...
void lowPowerClockSource(int lpcs)
{
  ...
  while (((NRF_CLOCK->LFCLKSTAT & CLOCK_LFCLKSTAT_STATE_Msk) >> CLOCK_LFCLKSTAT_STATE_Pos) == CLOCK_LFCLKSTAT_STATE_Running)
    ;

  NRF_CLOCK->LFCLKSRC = lpcs;
  NRF_CLOCK->EVENTS_LFCLKSTARTED = 0;
  NRF_CLOCK->TASKS_LFCLKSTART = 1;

  // Startup times:
  // - 32.768 kHz RC oscillator (32k RCOSC),         typical = 390us, max = 487us
  // - 32.768 kHz crystal oscillator (32k XOSC),     typical = 300ms, max = 1000ms
  // - 32.768 kHz Synthesized oscillator (32k SYNT), typical = 406us, max = 406us
